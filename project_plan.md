...
Our goal is to build something in between ExcaliDraw and Figma. You want something that's simple drag-and-drop of components from something like Tremor or shadcnstudio.com or other UI libraries that we can load into the system that are shadcn-based. You can drag and drop the modules on the page, resize them, and when you hover over them it will give you the option to see the code block and you can modify the code block to change the look.

We're not looking to build a functional app with this tool but to be able to wireframe and layout and do mock-ups of what the layout would look like for us to then feed it into a Claude Code session for it to actually build the app.

So our goal is to make a drag-and-drop of the components and blocks from these libraries and then being able to take that and have it build the layout. Being able to resize, move, change the text, change the look and feel a little bit. Change the data points so that we can simulate what the layout would look like using our pre-existing theme. The available components from the different UIs
